...
export default function ProductList() {
  ...
  // Ensure brands and categories are arrays before using them
  const filters = [
    {
      id: 'category',
      name: 'Category',
        ...
        <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
          ...
          {products.map((product) => (
            <div key={product.id} className="group relative border-solid border-2 p-2 border-gray-200">
              ...
              {user && product.stock > 0 && !product.deleted && (
                <button
                  onClick={(e) => handleQuickAddToCart(e, product)}
                  className="mt-2 w-full bg-indigo-600 text-white py-2 px-4 rounded-md hover:bg-indigo-700 transition-colors text-sm font-medium"
                >
                  Quick Add to Cart
                </button>
              )}
              ...
              {!user && product.stock > 0 && !product.deleted && (
                <button
                  ...
                  className="mt-2 w-full bg-gray-600 text-white py-2 px-4 rounded-md hover:bg-gray-700 transition-colors text-sm font-medium"
                >
